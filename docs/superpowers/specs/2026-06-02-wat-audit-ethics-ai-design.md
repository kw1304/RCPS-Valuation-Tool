...
비개발자용으로 "왜 그런지·무엇이 위험한지" 중심 기록. 구현은 자동 처리, 아래 도메인
판단·리스크만 사용자가 확인하면 됨.

### 11.1 강점 (이 설계가 안전한 이유)

- **검증된 뼈대 재사용**: accounting.py는 이미 실서비스 중(간헐적 빈답변 버그까지 수정
  완료, [[wat_tool_standard]]). SSE·세션·세마포어·타임아웃을 새로 짜지 않으니 회귀
  위험이 작다. 바뀌는 건 "무엇을 답하느냐"(프롬프트)뿐.
- **무결성 가드 승계**: "검색 안 했으면 URL 만들지 마라" 가드가 그대로 온다. 법령·
  질의회신은 가짜 출처가 특히 위험한 영역이라 이 가드가 핵심.
- **거울 구조**: 회계기준 툴과 범위가 안 겹치게 게이트를 반대로 세움 → 두 툴이 서로
  반쪽짜리가 안 됨.

### 11.2 도메인 정확성 — 반드시 짚을 점 (구현 시 프롬프트에 반영)

1. **윤리기준 인용번호 함정**: IESBA Code는 2018년 전면 재편(restructured)됐다. 독립성
   조항이 옛 `290.x` → 현행은 Part 4A/4B의 `R400번대·R600번대` 등으로 바뀜.
   섹션 5의 예시 `Code 290.N`은 **구 번호** — 구현 프롬프트에서 "현행 재편 번호 우선,
   불확실하면 단정 말고 원문 확인 권고"로 명시해야 함. fast 모드 환각 위험 1순위.
2. **법령 정식명칭·개정 주의**: 외부감사법 정식명칭은 「주식회사 등의 외부감사에 관한
   법률」(2018 전부개정, '신외감법'). 공인회계사법·외감법은 개정이 잦아 **fast 모드의
   조문번호가 옛 번호일 위험**이 회계기준보다 큼. → 법령 질문은 grounded(법제처 원문
   확인) 우선 권고를 프롬프트에 강제.
3. **품질관리기준 세대 교체**: 구 ISQC 1 → 현행 **ISQM 1·2(2022~)**, 한공회 채택본은
   품질관리기준(KQM/KQCS). 옛 ISQC로 답하지 않도록 명시.
4. **KSA=ISA 매핑**: 한국은 ISA를 그대로 채택(KSA 200~810). 국제기준 병기는 OK이나
   "한국 미채택 ISA 조항"을 한국 적용인 양 답하지 않도록 주의.

### 11.3 범위 경계 — 모호점과 결정

- **내부회계관리제도(ICFR)**: 신외감법상 상장사 ICFR 감사는 감사·외감법 영역 → **범위
  포함**으로 결정(섹션 2의 '감사기준' 축에 ICFR 감사·검토기준 명시 권고).
- **감리(감독) 절차 자체**: 감리지적사례는 검색 우선순위 2번으로 *근거*로 씀. 다만
  "감리 절차·제재 양정" 같은 감독행정 질문도 답할지 → **답함**(외감법·감독규정 영역).
- **순수 세무·법인세**: 범위 밖(거절). 회계감사 맥락이면 한정 답변.

### 11.4 기술 리스크 (가벼움, 자동 처리)

- **동시 실행 한도**: accounting.py 세마포어(동시 3)와 audit.py 세마포어가 **별개** →
  두 툴 합쳐 최대 6개 claude 프로세스 동시 가능. 기존 머신 부하 OK인지 가동 후 확인.
  필요시 공유 세마포어로 합칠 수 있음(후속).
- **법제처 국가법령정보 수집**: JS 동적 사이트라 WebSearch가 본문 전체를 못 긁을 수
  있음 → 조문 인용은 되되 "원문 링크 제공 + 핵심 발췌" 수준으로 기대치 설정.
- **DB 공유**: 같은 sqlite 파일에 `audit_conv` 테이블만 추가 → 회계기준 세션과 충돌
  없음(테이블 분리).

### 11.5 테스트 — 비개발자 검증 포인트

골든질문은 **사용자가 답을 직접 검증**할 수 있는 것으로 구성(코드는 못 봐도 답 맞는지는
판단 가능):
- "감사인 독립성 — 비감사용역 제한 근거는?" → 윤리규정 + 외감법 제25조(비감사용역)
  교차 인용 나오는지
- "핵심감사사항(KAM) 기준서?" → KSA 701
- "감사인 손해배상공동기금 근거?" → 공인회계사법 조문
- "부정에 대한 감사인 책임?" → KSA 240
이 4개가 의도대로 나오면 범위·인용·교차통합이 작동하는 것.

### 11.6 미해결·후속 (지금 안 정해도 됨)

- 음성/PDF 질문 업로드: 비범위(웹검색 기반 유지).
- 두 툴 세마포어 통합 여부: 가동 후 부하 보고 결정.
- 법령 개정 추적 자동화: 현재는 grounded 검색에 의존(별도 모니터링 없음).
