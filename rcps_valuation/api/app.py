...
@app.route("/api/subsequent_attribution", methods=["POST"])
def subsequent_attribution():
    """후속측정 평가손익 4-factor attribution.

    K-IFRS 1109.B5.1.1 FVTPL 분류 RCPS의 분기·반기 공정가치 변동을
    (i) 주가 (ii) 변동성 (iii) 금리 (iv) 시간경과 4개 factor로 분해.

    body: {"prev": {...params}, "curr": {...params}, "steps": int (optional)}
    각 factor를 prev → curr 단독 변경한 fv 차이로 효과 산출. 잔차는 상호작용.
    """
    try:
        from copy import deepcopy
        data = request.json
        prev_params = parse_params(data["prev"])
        curr_params = parse_params(data["curr"])
        steps_req = int(data.get("steps", 0)) or None
        if steps_req:
            steps = max(1, min(int(steps_req), 520))
        else:
            steps = max(int(round(prev_params.T * 12)), 12)

        def _fv(p):
            return tf_rcps(p, steps=steps, bond_discrete=False).get("fair_value")

        fv_prev = _fv(prev_params)
        fv_curr_steps = max(int(round(curr_params.T * 12)), 12) if not steps_req else steps
        fv_curr = tf_rcps(curr_params, steps=fv_curr_steps, bond_discrete=False).get("fair_value")

        def with_change(**kwargs):
            p = deepcopy(prev_params)
            for k, v in kwargs.items():
                setattr(p, k, v)
            return _fv(p)

        fv_S = with_change(stock_price=curr_params.stock_price)
        fv_sigma = with_change(volatility=curr_params.volatility)
        fv_r = with_change(risk_free_rate=curr_params.risk_free_rate,
                           credit_spread=curr_params.credit_spread)
        # 시간경과: valuation_date만 변경 (T 자동 감소)
        fv_T = with_change(valuation_date=curr_params.valuation_date)

        effect_S = fv_S - fv_prev
        effect_sigma = fv_sigma - fv_prev
        effect_r = fv_r - fv_prev
        effect_T = fv_T - fv_prev
        actual_change = fv_curr - fv_prev
        interaction = actual_change - (effect_S + effect_sigma + effect_r + effect_T)

        return jsonify({
            "status": "ok",
            "fv_prev": round(fv_prev),
            "fv_curr": round(fv_curr),
            "change": round(actual_change),
            "change_pct": round((actual_change / fv_prev * 100), 2) if fv_prev else None,
            "effects": {
                "stock_price": round(effect_S),
                "volatility": round(effect_sigma),
                "interest_rates": round(effect_r),
                "time_decay": round(effect_T),
                "interaction": round(interaction),
            },
            "prev_inputs": {
                "valuation_date": str(prev_params.valuation_date),
                "stock_price": prev_params.stock_price,
                "volatility": prev_params.volatility,
                "risk_free_rate": prev_params.risk_free_rate,
                "credit_spread": prev_params.credit_spread,
            },
            "curr_inputs": {
                "valuation_date": str(curr_params.valuation_date),
                "stock_price": curr_params.stock_price,
                "volatility": curr_params.volatility,
                "risk_free_rate": curr_params.risk_free_rate,
                "credit_spread": curr_params.credit_spread,
            },
            "steps_used": steps,
            "interpretation": (
                "Δfv = 주가효과 + 변동성효과 + 금리효과 + 시간경과효과 + 상호작용. "
                "상호작용은 factor 간 결합 효과(통상 |Δfv|의 5% 이내)."
            ),
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e), "trace": traceback.format_exc()}), 400
