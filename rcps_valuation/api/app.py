...
def _find_bs_row_at(ticker_obj, target_date_str, row_names):
    """target_date 이하 가장 가까운 period의 BS 행값(첫 번째 발견 row_name) 추출.
    return: (period_end_str, value) 또는 None.
    """
    try:
        import pandas as pd
    except Exception:
    ...
    for bs_attr in ('quarterly_balance_sheet', 'balance_sheet'):
        ...
        if bs is None or bs.empty:
            continue
        for rn in row_names:
            if rn not in bs.index:
                continue
            valid = []
            for c in bs.columns:
                try:
                    if pd.Timestamp(c) <= target and pd.notna(bs.loc[rn, c]):
                        valid.append(c)
                except Exception:
                    continue
            if valid:
                best = max(valid)
                return (str(best.date()), float(bs.loc[rn, best]))
    return None


def _find_is_row_at(ticker_obj, target_date_str, row_names):
    """손익계산서(연간 우선, 분기 fallback) 에서 target_date 이하 가장 가까운 period 값."""
    try:
        import pandas as pd
    except Exception:
        return None
    target = pd.Timestamp(target_date_str)
    for is_attr in ('income_stmt', 'quarterly_income_stmt', 'financials', 'quarterly_financials'):
        df = getattr(ticker_obj, is_attr, None)
        ...
        for rn in row_names:
            ...
            for c in df.columns:
                try:
                    if pd.Timestamp(c) <= target and pd.notna(df.loc[rn, c]):
                        valid.append(c)
                except Exception:
                    continue
            if valid:
                best = max(valid)
                return (str(best.date()), float(df.loc[rn, best]))
    return None


@app.route("/api/dcf/auto_inputs", methods=["POST"])
def dcf_auto_inputs():
    """평가대상 상장사 BS·IS·CF 항목을 평가기준일 기준으로 일괄 조회 → DCF 입력칸 자동 채움.

    body: {ticker, date}
    returns: {status, ticker, name, period_end, net_debt, shares, non_op_assets,
              total_revenue, operating_income, depreciation, components:{...}, sources:{...}}
    """
    data = request.get_json(force=True) or {}
    code = str(data.get("ticker") or "").strip()
    date_str = (data.get("date") or "").strip() or None
    if not code:
        return jsonify({"status": "error", "message": "ticker 없음"}), 200
    try:
        import yfinance as yf
    except Exception:
        return jsonify({"status": "error", "message": "yfinance 미설치"}), 200
    ...
    try:
        ...
        # BS 항목 추출 (period_end ≤ valuation_date)
        d = date_str or datetime.now().date().isoformat()
        td   = _find_bs_row_at(t, d, ('Total Debt',))
        cash = _find_bs_row_at(t, d, ('Cash And Cash Equivalents', 'Cash Cash Equivalents And Short Term Investments', 'Cash'))
        stinv = _find_bs_row_at(t, d, ('Short Term Investments', 'Other Short Term Investments'))
        ltinv = _find_bs_row_at(t, d, ('Long Term Equity Investment', 'Investments And Advances', 'Long Term Investments'))
        sh_bs = _find_bs_row_at(t, d, ('Share Issued', 'Ordinary Shares Number'))

        # IS 항목 추출
        rev   = _find_is_row_at(t, d, ('Total Revenue', 'Operating Revenue'))
        opinc = _find_is_row_at(t, d, ('Operating Income', 'EBIT'))
        ...
        try:
            ...
            for cf_attr in ('cashflow', 'quarterly_cashflow'):
                cf = getattr(t, cf_attr, None)
                if cf is None or cf.empty:
                    continue
                for rn in ('Depreciation And Amortization', 'Depreciation Amortization Depletion', 'Depreciation'):
                    if rn in cf.index:
                        valid = [c for c in cf.columns if pd.Timestamp(c) <= target and pd.notna(cf.loc[rn, c])]
                        if valid:
                            best = max(valid)
                            dep = (str(best.date()), float(cf.loc[rn, best]))
                            break
                if dep:
                    break
        except Exception:
        ...
        net_debt = None
        if td_v is not None:
            net_debt = td_v - (cash_v or 0) - (stinv_v or 0)
        # 비영업자산 근사 = 단기금융상품 + 장기투자(관계회사 등)
        non_op = None
        if stinv_v is not None or ltinv_v is not None:
            non_op = (stinv_v or 0) + (ltinv_v or 0)
        ...
        return jsonify({
            ...
            "requested_date": date_str,
            "net_debt": net_debt,
            "shares": v(sh_bs) or shares_info,
            "non_op_assets": non_op,
            "total_revenue": v(rev),
            "operating_income": v(opinc),
            "depreciation": v(dep),
            "components": {
                "total_debt": td_v,
                "cash_and_equiv": cash_v,
                "short_term_investments": stinv_v,
                "long_term_investments": ltinv_v,
            },
            "sources": {
                "total_debt": src(td),
                "cash": src(cash),
                "short_term_investments": src(stinv),
                "long_term_investments": src(ltinv),
                "shares_bs": src(sh_bs),
                "revenue": src(rev),
                "operating_income": src(opinc),
                "depreciation": src(dep),
            },
        })
    except Exception as e:  # noqa: BLE001
        return jsonify({"status": "error", "ticker": code, "yahoo_symbol": ysym, "message": str(e)[:200]}), 200
