"""
Golden-reference test: TF 노드희석_구분할인 모형
Expected fair_value ≈ 10,799.53  (reference answer)
"""
import sys, os
...
def build_params():
    ...
    return RCPSParams(
        ...
        volatility=0.3,
        risk_free_rate=0.02,
        credit_spread=0.01944,       # 0.02 + 0.01944 ≈ flat; unused when kd_curve given
        dividend_yield=0.0,
        common_shares=100,
        rcps_shares=20,
    )
...
def test_golden_reference():
    ...
    print(f"[golden] fair_value = {fv}  (expected ~10800)")
    print(f"  equity_component  = {result['equity_component']}")
    print(f"  bond_component    = {result['bond_component']}")
    print(f"  dilution_applied  = {result['dilution_applied']}")
    print(f"  diluted_stock_price_0 = {result['diluted_stock_price_0']}")
    assert abs(fv - 10800) <= 1.0, (
        f"Golden-reference FAILED: fair_value={fv}, expected ≈10800 (tol ±1)"
    )
    print("[golden] PASSED")


def test_backward_compat():
    """No share counts, no curves — must return a number without error."""
    params = RCPSParams(
        issue_date=date(2020, 1, 1),
        valuation_date=date(2020, 1, 1),
        maturity_date=date(2025, 1, 1),
    ...
    result = tf_rcps(params, steps=60)
    fv = result["fair_value"]
    print(f"[compat] fair_value = {fv}  dilution_applied={result['dilution_applied']}")
